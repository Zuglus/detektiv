...
- **Удалены старые CSS классы**: card, card-dark, card-colored, principle-card, card-emergency, card-accent, gradient-card-isolated
- **Очищен cards.css**: С ~240 строк до ~70 строк (удалено 170+ строк legacy кода)
- **Сохранена совместимость**: trust-indicator, pricing-card остались (активно используются)
- **Проверено**: Сборка успешна, TypeScript без ошибок, все 82 страницы собраны

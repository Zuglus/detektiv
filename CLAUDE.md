...
- Где вид менять не надо, растёт только область нажатия: `py-6` с отрицательным полем
  (`-my-6`; у строчного блока с `mb-6` — `-mt-6` вместо него), в панели — `py-6 -my-3`
  (пилюли рубрик — 44px, панель 45px). У видимой плашки — `::after` с `-inset-y`,
  считая от внутреннего края рамки

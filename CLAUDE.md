...
- Форма нажимаемого (заливка, рамка) видна в покое, наведение её только усиливает: на
  ...
  как рубрика на карточке статьи, наведение `primary-100`; рубрика текущего раздела
  (`aria-current="location"`, ставит mobile-menu.js) — `primary-200` с текстом `primary-800`,
  на тон гуще, кольцо фокуса на ней `primary-700`. Тёмная `primary-700` с белым текстом
  перетягивала внимание: панель прилипает к верху и всё время в поле зрения
...
- JS минимален: только mobile-menu (меню на телефоне, состояние ленты; панель рубрик —
  подсветка рубрики текущего раздела, сдвиг ряда к ней, докрутка по Tab), scroll-reveal, метрика
...
- Tailwind читает и комментарии в шаблонах: класс, названный в комментарии, попадает в CSS.
  Писать там полное имя (`max-lg:last:pr-16`, не `last:pr-16`) — иначе в CSS ляжет правило,

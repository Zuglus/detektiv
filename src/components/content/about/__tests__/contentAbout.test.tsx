...
// We'll use the real about.json since mocking doesn't seem to work properly
...
describe("ContentAbout", () => {
  it("renders hero section for Russian language", () => {
    ...
    expect(
      screen.getByText("Профессиональные детективные услуги с 2010 года"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        "Опыт, конфиденциальность и результат — основа нашей работы",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("renders hero section for English language", () => {
    ...
    expect(
      screen.getByText("Professional detective services since 2010"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        "Experience, reliability and results — the foundation of our work",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("renders founder section", () => {
    ...
    expect(screen.getByText("Основание агентства")).toBeInTheDocument();
    expect(
      screen.getByText(/Детективное агентство создано/),
    ).toBeInTheDocument();
  ...
  it("renders warning section", () => {
    ...
    expect(
      screen.getByText(/Рынок детективных услуг насыщен/),
    ).toBeInTheDocument();

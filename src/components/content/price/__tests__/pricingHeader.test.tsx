...
describe("PricingHeader", () => {
  const mockIntroList = {
    ru: ["Детальная консультация", "Понятные условия", "Гарантия результата"],
    en: ["Detailed consultation", "Clear terms", "Result guarantee"],
  };
  ...
  describe("rendering", () => {
    ...
    it("displays heading in Russian", () => {
      render(<PricingHeader lang="ru" introList={mockIntroList} />);
      expect(
        screen.getByText("Прозрачные цены на детективные услуги"),
      ).toBeInTheDocument();
    });
    ...
    it("displays heading in English", () => {
      render(<PricingHeader lang="en" introList={mockIntroList} />);
      expect(
        screen.getByText("Transparent Detective Service Pricing"),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("intro list items", () => {
    ...
    it("renders correct number of items", () => {
      const { container } = render(
        <PricingHeader lang="ru" introList={mockIntroList} />,
      );
  ...
  describe("edge cases", () => {
    it("handles empty intro list", () => {
      ...
      const { container } = render(
        <PricingHeader lang="ru" introList={emptyIntroList} />,
      );
      const items = container.querySelectorAll(".space-y-3");
    ...
    it("handles many items in intro list", () => {
      ...
      const { container } = render(
        <PricingHeader lang="ru" introList={manyItemsList} />,
      );
      const items = container.querySelectorAll(".space-y-3");

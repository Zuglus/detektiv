...
jest.mock("@/components/ui/UnifiedCard", () => {
  return function MockUnifiedCard({
    ...
    return (
      <div
        data-testid={`unified-card-${variant || "default"}`}
...
jest.mock("@/components/ui/UnifiedButton", () => {
  return function MockUnifiedButton({
    ...
    if (as === "link") {
      return (
        <a data-testid="unified-button" {...props}>
          {children}
        </a>
      );
    }
    return (
      <button data-testid="unified-button" {...props}>
        {children}
      </button>
    );
  };
...
jest.mock("@/components/ui/ScrollReveal", () => {
  return function MockScrollReveal({
    children,
  }: {
    children: React.ReactNode;
  }) {
...
// Mock JSON data
jest.mock(
  "@/components/content/main/main.json",
  () => ({
    subheader: {
    ...
    serviceList: [
      {
        ...
        en: {
          title: "dossier",
          text: "Collection of relevant information about a person or object",
        },
      },
      {
      ...
      },
      {
        ru: {
          ...
          text: "Одноклассников, попутчиков",
        },
        en: {
          title: "private search for people",
          text: "Classmates, fellow travelers",
        },
      },
      {
        ru: {
          title: "частный розыск должников и алиментщиков",
          text: "Оперативный поиск",
        },
      ...
      },
      {
        ru: {
          ...
          text: "Сбор и проверка сведений",
        },
        en: {
          title: "business partner check",
          text: "Collection and verification",
        },
      },
      {
      ...
      },
      {
        ru: {
          ...
          text: "Сбор и проверка информации",
        },
        en: {
          title: "information abroad",
          text: "Collection and verification",
        },
      },
    ],
    ...
    benefitsList: [
      {
        ...
        en: {
          title: "we always adhere to strict confidentiality",
          text: "After the information is passed to the client, all data stored with us is obligatorily destroyed.",
        },
      },
      {
        id: 2,
        ru: {
          ...
          text: "Взаимодействуем с коллегами по всему миру",
        },
        en: {
          title: "we apply well-established connections",
          text: "We cooperate with colleagues worldwide",
        },
      },
      {
        id: 3,
        ru: {
          ...
          text: "Идем в ногу со временем",
        },
        en: {
          title: "we constantly raise our level of professional development",
          text: "We keep up with the times",
        },
      },
      {
        id: 4,
        ru: {
          ...
          text: "Конфиденциальность и экономия время",
        },
        en: {
          title: "we work remotely - online",
          text: "Confidentiality and time-saving",
        },
      },
    ],
...
jest.mock(
  ...
  () => ({
    ...
    email: {
      ...
      directName: "detectivegroznyi@gmail.com",
    },
...
describe("ContentMain", () => {
  it("renders main content for Russian language", () => {
    ...
    expect(
      screen.getByText("предлагаем помощь в решении ваших вопросов"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("процесс заказа выглядит следующим образом:"),
    ).toBeInTheDocument();
  });
  ...
  it("renders main content for English language", () => {
    ...
    expect(
      screen.getByText("We are online and ready to help:"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("we offer help in solving your issues"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("the order process is as follows:"),
    ).toBeInTheDocument();
  });
  ...
  it("renders service cards", () => {
    ...
    expect(
      screen.getByText("частный розыск должников и алиментщиков"),
    ).toBeInTheDocument();
    expect(screen.getByText("проверка персонала")).toBeInTheDocument();
    expect(
      screen.getByText("проверка фирм и деловых партнеров"),
    ).toBeInTheDocument();
  ...
  it("renders order process steps", () => {
    ...
    expect(
      screen.getByText(/Бесплатная предварительная консультация/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Получение вводных данных от Заказчика/),
    ).toBeInTheDocument();
  });
  ...
  it("renders benefits list", () => {
    ...
    expect(
      screen.getByText("всегда придерживаемся строгой конфиденциальности"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/После передачи информации заказчику/),
    ).toBeInTheDocument();
  });

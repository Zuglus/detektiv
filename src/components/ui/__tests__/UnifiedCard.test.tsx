...
describe("UnifiedCard", () => {
  ...
  it("applies correct variant styles", () => {
    ...
    variants.forEach((variant) => {
      const { rerender } = render(
        <UnifiedCard variant={variant} data-testid="card">
          Content
        </UnifiedCard>,
      );
      const card = screen.getByTestId("card");
  ...
  it("applies correct size classes", () => {
    const { rerender } = render(
      <UnifiedCard size="compact" data-testid="card">
        Content
      </UnifiedCard>,
    );
    ...
    rerender(
      <UnifiedCard size="default" data-testid="card">
        Content
      </UnifiedCard>,
    );
    ...
    rerender(
      <UnifiedCard size="large" data-testid="card">
        Content
      </UnifiedCard>,
    );
  ...
  it("applies bordered styling when bordered prop is true", () => {
    render(
      <UnifiedCard bordered data-testid="card">
        Content
      </UnifiedCard>,
    );
  ...
  it("applies interactive styles when interactive prop is true", () => {
    render(
      <UnifiedCard interactive data-testid="card">
        Content
      </UnifiedCard>,
    );
  ...
  it("renders emergency overlay for emergency variant", () => {
    ...
    const overlay = card.querySelector(
      ".bg-gradient-to-br.from-error-50\\/30.to-accent-50\\/30",
    );
  ...
  it("renders accent overlay for accent variant", () => {
    render(
      <UnifiedCard variant="accent" data-testid="card">
        Content
      </UnifiedCard>,
    );
  ...
  it("renders principle background for principle variant", () => {
    render(
      <UnifiedCard variant="principle" data-testid="card">
        Content
      </UnifiedCard>,
    );
    ...
    const background = card.querySelector(
      ".bg-gradient-to-br.from-secondary-800\\/10.to-primary-600\\/10",
    );
  ...
  it("applies shadow styles correctly for different variants", () => {
    const { rerender } = render(
      <UnifiedCard variant="dark" data-testid="card">
        Content
      </UnifiedCard>,
    );

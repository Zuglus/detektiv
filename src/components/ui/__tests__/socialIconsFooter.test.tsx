...
describe("SocialIconsFooter", () => {
  it("renders all social icons with proper alignment", () => {
    ...
    expect(
      screen.getByLabelText(/Связаться с нами через Телефон/),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText(/Связаться с нами через Email/),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText(/Связаться с нами через Telegram/),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText(/Связаться с нами через WhatsApp/),
    ).toBeInTheDocument();
  });
  ...
  it("has proper external link attributes", () => {
    ...
    const telegramLink = screen.getByLabelText(
      /Связаться с нами через Telegram/,
    );
